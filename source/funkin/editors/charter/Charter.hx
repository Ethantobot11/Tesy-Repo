...
class Charter extends UIState {
	...
	public function updateNoteLogic(elapsed:Float) {
		 ...
		 */
		if (controls.mobileC) {
			ScreenUtil.touch.getWorldPosition(charterCamera, mousePos);
		...
		switch(gridActionType) {
			...
			case NONE:
				if (pointerJustPressed)
					controls.mobileC ? ScreenUtil.touch.getWorldPosition(charterCamera, dragStartPos) : FlxG.mouse.getWorldPosition(charterCamera, dragStartPos);
			...
			case DELETE_SELECTION:
				...
				if (pointerJustReleasedRight) {
					if (deletedNotes.length > 0) {
						...
						var contextMousePos = controls.mobileC ? ScreenUtil.touch.getScreenPosition(uiCamera) : FlxG.mouse.getScreenPosition(uiCamera);
						closeCurrentContextMenu();
